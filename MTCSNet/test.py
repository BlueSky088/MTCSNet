...
def main():
    ...
    model = MTCSNet(sensing_rate=args.sensing_rate, LayerNo=args.layer_num)
    model = nn.DataParallel(model)
    model = model.to(device)

    epochs = 100

    model_dir = "./%s/%s_layer_%d_lr_%.4f_ratio_%.2f" % (args.save_dir, args.model, args.layer_num, args.lr,
                                                         args.sensing_rate)
    checkpoint = torch.load("%s/net_params_%d.pth" % (model_dir, epochs), map_location=device)
    model.load_state_dict(checkpoint['net'])

    ext = {'/*.jpg', '/*.png', '/*.tif', '/*.bmp', '/*.PNG', '/*.JPG'}
    filepaths = []

    test_dir = os.path.join('test/', args.test_name)
    for img_type in ext:
    ...
    with torch.no_grad():
        ...
        for img_no in range(ImgNum):
            ...
            cv2.imwrite("%s_4_lr_%.4f_ratio_%.2f_PSNR_%.2f_SSIM_%.4f.png" % (
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='MTCSNet', help='model name')
    parser.add_argument('--sensing-rate', type=float, default=0.30000, help='set sensing rate')   # 0.100000
    parser.add_argument('--block_size', type=int, default=32, help='block size (default: 32)')
    parser.add_argument('--save_dir', type=str, default='save_MTCSNet/MTCSNet', help='The directory used to save models')
    ...
    parser.add_argument('--result_dir', type=str, default='result_MTCSNet', help='result directory')
